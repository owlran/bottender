...
export async function setWebhook(_webhook, eventTypes = []) {
  try {
    ...
    if (eventTypes.length !== 0) {
      result = await client.setWebhook(webhook, eventTypes);
    } else {
      result = await client.setWebhook(webhook);
    }
    invariant(result.data.ok, 'Setting for webhook is failed');
...
export async function deleteWebhook() {
  try {
    ...
    const client = ViberClient.connect(accessToken);
    const { data: { ok } } = await client.removeWebhook();
    invariant(ok, 'Deleting Viber webhook is failed');
